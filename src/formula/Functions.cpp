...
QHash<QString, Fn> &fnMap() {
    static QHash<QString, Fn> m = [] {
        ...
        // GAMMA(x): hàm gamma Γ(x). GAMMALN(x): logarit tự nhiên của Γ(x), x > 0.
        r["GAMMA"] = [need](const Args &a) {
            need(a,1,"GAMMA"); double x = toNumber(a[0]);
            if (x <= 0 && x == std::floor(x)) throw FormulaError(QStringLiteral("GAMMA: không xác định tại số nguyên <= 0"), ERR_NUM);
            return Value::number(std::tgamma(x));
        };
        r["GAMMALN"] = [need](const Args &a) {
            need(a,1,"GAMMALN"); double x = toNumber(a[0]);
            if (x <= 0) throw FormulaError(QStringLiteral("GAMMALN: cần x > 0"), ERR_NUM);
            return Value::number(std::lgamma(x));
        };
        // NORMSDIST(z): hàm phân phối tích lũy chuẩn tắc. NORMSINV(p): nghịch đảo (Acklam).
        r["NORMSDIST"] = [need](const Args &a) { need(a,1,"NORMSDIST"); return Value::number(0.5 * std::erfc(-toNumber(a[0]) / std::sqrt(2.0))); };
        r["NORMSINV"] = [need](const Args &a) {
            need(a,1,"NORMSINV"); double p = toNumber(a[0]);
            if (p <= 0 || p >= 1) throw FormulaError(QStringLiteral("NORMSINV: p trong (0,1)"), ERR_NUM);
            // Thuật toán xấp xỉ Acklam cho phân vị chuẩn tắc.
            ...
            double pl = 0.02425, ph = 1 - pl, q, r, x;
            if (p < pl) { q = std::sqrt(-2*std::log(p)); x = (((((C[0]*q+C[1])*q+C[2])*q+C[3])*q+C[4])*q+C[5]) / ((((D[0]*q+D[1])*q+D[2])*q+D[3])*q+1); }
        ...
        // === Nhóm phân phối xác suất (Spec 12) ===
        r["BINOMDIST"] = [need](const Args &a) { need(a, 4, "BINOMDIST"); int k=toInt(a[0]); int n=toInt(a[1]); double p=toNumber(a[2]); bool cum=toInt(a[3])!=0; if (k<0||n<0||k>n||p<0.0||p>1.0) throw FormulaError(QStringLiteral("BINOMDIST: tham so khong hop le"), ERR_NUM); auto pmf=[&](int kk){ double c=1.0; for(int i=0;i<kk;i++) c=c*(double)(n-i)/(double)(i+1); return c*std::pow(p,(double)kk)*std::pow(1.0-p,(double)(n-kk)); }; if(cum){ double s=0.0; for(int j=0;j<=k;j++) s+=pmf(j); return Value::number(s);} return Value::number(pmf(k)); };
        r["POISSON"] = [need](const Args &a) { need(a, 3, "POISSON"); int x=toInt(a[0]); double mean=toNumber(a[1]); bool cum=toInt(a[2])!=0; if (x<0||mean<0.0) throw FormulaError(QStringLiteral("POISSON: tham so khong hop le"), ERR_NUM); auto pmf=[&](int kk){ double f=1.0; for(int i=2;i<=kk;i++) f*=(double)i; return std::exp(-mean)*std::pow(mean,(double)kk)/f; }; if(cum){ double s=0.0; for(int j=0;j<=x;j++) s+=pmf(j); return Value::number(s);} return Value::number(pmf(x)); };
        r["EXPONDIST"] = [need](const Args &a) { need(a, 3, "EXPONDIST"); double x=toNumber(a[0]); double lambda=toNumber(a[1]); bool cum=toInt(a[2])!=0; if (x<0.0||lambda<=0.0) throw FormulaError(QStringLiteral("EXPONDIST: tham so khong hop le"), ERR_NUM); if(cum) return Value::number(1.0-std::exp(-lambda*x)); return Value::number(lambda*std::exp(-lambda*x)); };
        r["WEIBULL"] = [need](const Args &a) { need(a, 4, "WEIBULL"); double x=toNumber(a[0]); double alpha=toNumber(a[1]); double beta=toNumber(a[2]); bool cum=toInt(a[3])!=0; if (x<0.0||alpha<=0.0||beta<=0.0) throw FormulaError(QStringLiteral("WEIBULL: tham so khong hop le"), ERR_NUM); double t=std::pow(x/beta,alpha); if(cum) return Value::number(1.0-std::exp(-t)); return Value::number((alpha/std::pow(beta,alpha))*std::pow(x,alpha-1.0)*std::exp(-t)); };
        r["NEGBINOMDIST"] = [need](const Args &a) { need(a, 3, "NEGBINOMDIST"); int f=toInt(a[0]); int s=toInt(a[1]); double p=toNumber(a[2]); if (f<0||s<1||p<0.0||p>1.0) throw FormulaError(QStringLiteral("NEGBINOMDIST: tham so khong hop le"), ERR_NUM); int n=f+s-1; int kk=f; double c=1.0; for(int i=0;i<kk;i++) c=c*(double)(n-i)/(double)(i+1); return Value::number(c*std::pow(p,(double)s)*std::pow(1.0-p,(double)f)); };
        r["HYPGEOMDIST"] = [need](const Args &a) { need(a, 4, "HYPGEOMDIST"); int ss=toInt(a[0]); int ns=toInt(a[1]); int ps=toInt(a[2]); int np=toInt(a[3]); if (ss<0||ss>ns||ss>ps||ns-ss>np-ps||ns<0||ps<0||np<0||ns>np||ps>np) throw FormulaError(QStringLiteral("HYPGEOMDIST: tham so khong hop le"), ERR_NUM); auto comb=[&](int n,int k)->double{ if(k<0||k>n) return 0.0; double c=1.0; for(int i=0;i<k;i++) c=c*(double)(n-i)/(double)(i+1); return c; }; double denom=comb(np,ns); if(denom==0.0) throw FormulaError(QStringLiteral("HYPGEOMDIST: chia cho 0"), ERR_DIV0); return Value::number(comb(ps,ss)*comb(np-ps,ns-ss)/denom); };
        // === Nhóm toán/thống kê bổ sung (Spec 12) ===
        r["CONFIDENCE"] = [need](const Args &a) { need(a, 3, "CONFIDENCE"); double alpha = toNumber(a[0]), sd = toNumber(a[1]), size = toNumber(a[2]); if (alpha <= 0 || alpha >= 1) throw FormulaError(QStringLiteral("CONFIDENCE: alpha phải trong (0,1)"), ERR_NUM); if (sd <= 0) throw FormulaError(QStringLiteral("CONFIDENCE: độ lệch chuẩn phải > 0"), ERR_NUM); if (size < 1) throw FormulaError(QStringLiteral("CONFIDENCE: kích thước phải >= 1"), ERR_NUM); double target = 1.0 - alpha / 2.0; double lo = 0.0, hi = 10.0; for (int i = 0; i < 200; ++i) { double m = (lo + hi) / 2.0; double v = 0.5 * std::erfc(-m / std::sqrt(2.0)); if (v < target) lo = m; else hi = m; } double z = (lo + hi) / 2.0; return Value::number(z * sd / std::sqrt(size)); };
        r["TRIMMEAN"] = [need](const Args &a) { need(a, 2, "TRIMMEAN"); auto v = numbers(std::vector<Value>{a[0]}); double percent = toNumber(a[1]); if (percent < 0 || percent >= 1) throw FormulaError(QStringLiteral("TRIMMEAN: percent phải trong [0,1)"), ERR_NUM); if (v.empty()) throw FormulaError(QStringLiteral("TRIMMEAN: cần dữ liệu số"), ERR_NUM); std::sort(v.begin(), v.end()); int n = int(v.size()); int trim = int(std::floor(double(n) * percent / 2.0)) * 2; int k = trim / 2; double sum = 0; int cnt = 0; for (int i = k; i < n - k; ++i) { sum += v[i]; ++cnt; } if (cnt == 0) throw FormulaError(QStringLiteral("TRIMMEAN: không còn dữ liệu sau khi cắt"), ERR_NUM); return Value::number(sum / cnt); };
        r["MODE"] = [](const Args &a) { auto v = numbers(a); if (v.empty()) throw FormulaError(QStringLiteral("MODE: cần dữ liệu số"), ERR_NA); double best = 0; int bestCount = 0; for (size_t i = 0; i < v.size(); ++i) { int c = 0; for (size_t j = 0; j < v.size(); ++j) if (v[j] == v[i]) ++c; if (c > bestCount) { bestCount = c; best = v[i]; } } if (bestCount < 2) throw FormulaError(QStringLiteral("MODE: không có giá trị lặp lại"), ERR_NA); return Value::number(best); };
        r["SUMX2MY2"] = [need](const Args &a) { need(a, 2, "SUMX2MY2"); auto x = numbers(std::vector<Value>{a[0]}); auto y = numbers(std::vector<Value>{a[1]}); if (x.size() != y.size()) throw FormulaError(QStringLiteral("SUMX2MY2: hai vùng phải cùng kích thước"), ERR_NA); double s = 0; for (size_t i = 0; i < x.size(); ++i) s += x[i]*x[i] - y[i]*y[i]; return Value::number(s); };
        r["SUMX2PY2"] = [need](const Args &a) { need(a, 2, "SUMX2PY2"); auto x = numbers(std::vector<Value>{a[0]}); auto y = numbers(std::vector<Value>{a[1]}); if (x.size() != y.size()) throw FormulaError(QStringLiteral("SUMX2PY2: hai vùng phải cùng kích thước"), ERR_NA); double s = 0; for (size_t i = 0; i < x.size(); ++i) s += x[i]*x[i] + y[i]*y[i]; return Value::number(s); };
        r["SUMXMY2"] = [need](const Args &a) { need(a, 2, "SUMXMY2"); auto x = numbers(std::vector<Value>{a[0]}); auto y = numbers(std::vector<Value>{a[1]}); if (x.size() != y.size()) throw FormulaError(QStringLiteral("SUMXMY2: hai vùng phải cùng kích thước"), ERR_NA); double s = 0; for (size_t i = 0; i < x.size(); ++i) { double d = x[i] - y[i]; s += d*d; } return Value::number(s); };
        ...
        r["CONVERT"] = [need](const Args &a) {
            ...
            static const QHash<QString, U> units = {
                ...
                // Diện tích (mét vuông)
                {"m2",{3,1}}, {"km2",{3,1e6}}, {"cm2",{3,1e-4}}, {"ft2",{3,0.09290304}}, {"in2",{3,0.00064516}}, {"ha",{3,10000}},
                // Thể tích (lít)
                {"l",{4,1}}, {"ml",{4,0.001}}, {"m3",{4,1000}}, {"gal",{4,3.785411784}},
                {"qt",{4,0.946352946}}, {"pt",{4,0.473176473}}, {"ft3",{4,28.316846592}}, {"in3",{4,0.016387064}},
                // Áp suất (Pascal)
                {"Pa",{5,1.0}}, {"kPa",{5,1000.0}}, {"bar",{5,100000.0}}, {"atm",{5,101325.0}}, {"mmHg",{5,133.322}}, {"psi",{5,6894.757293168}},
                // Năng lượng (Joule)
                {"J",{6,1.0}}, {"kJ",{6,1000.0}}, {"cal",{6,4.184}}, {"kcal",{6,4184.0}}, {"Wh",{6,3600.0}}, {"kWh",{6,3600000.0}}, {"eV",{6,1.602176634e-19}},
                // Công suất (Watt)
                {"W",{7,1.0}}, {"kW",{7,1000.0}}, {"HP",{7,745.69987158227}}, {"PS",{7,735.49875}},
                // Lực (Newton)
                {"N",{8,1.0}}, {"kN",{8,1000.0}}, {"dyn",{8,1e-5}}, {"lbf",{8,4.4482216152605}},
            };
